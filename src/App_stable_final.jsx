...
function App() {
  ...
  const topPattern = useMemo(() => {
    ...
    problems.forEach((problem) => {
      const pattern = safeText(problem.pattern).trim();
      if (!pattern) return;
  ...
  const topTopic = useMemo(() => {
    const counts = {};
    problems.forEach((problem) => {
      const topic = safeText(problem.topic).trim();
      if (!topic) return;
  ...
  const filteredProblems = problems
    .filter((problem) => {
      const searchableText = `${safeText(problem.name)} ${safeText(problem.number)} ${safeText(problem.difficulty)} ${safeText(problem.pattern)} ${safeText(problem.topic)} ${safeText(problem.notes)} ${safeText(problem.mistakes)}`.toLowerCase();
  ...
  const editProblem = (problem) => {
    ...
    setForm({
      name: safeText(problem.name),
      number: safeText(problem.number),
      difficulty: safeText(problem.difficulty) || "Easy",
      pattern: safeText(problem.pattern),
      topic: safeText(problem.topic),
      notes: safeText(problem.notes),
      mistakes: safeText(problem.mistakes),
  ...
  return (
    <div className="app">
      ...
      <main>
        <form onSubmit={handleSubmit} className={`form ${editId !== null ? "editing" : ""}`}>
          <h2>{editId !== null ? "Edit Problem" : "Add Problem"}</h2>

          <input type="text" placeholder="Problem name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          <input type="text" placeholder="LeetCode number" value={form.number} onChange={(e) => setForm({ ...form, number: e.target.value })} />
          ...
          <input type="text" placeholder="Pattern e.g. Two Pointers" value={form.pattern} onChange={(e) => setForm({ ...form, pattern: e.target.value })} />
          <input type="text" placeholder="Topic e.g. Arrays" value={form.topic} onChange={(e) => setForm({ ...form, topic: e.target.value })} />
          <textarea placeholder="Notes for future revision" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
          <textarea placeholder="Mistakes you made" value={form.mistakes} onChange={(e) => setForm({ ...form, mistakes: e.target.value })} />

          <label className="checkbox">
            <input type="checkbox" checked={form.revision} onChange={(e) => setForm({ ...form, revision: e.target.checked })} />
            Mark for revision
          </label>
          ...
          <button type="submit">{editId !== null ? "Save Changes" : "Add Problem"}</button>
        ...
        <section className="list">
          <div className="filters">
            <input type="text" placeholder="Search by name, number, pattern, topic, notes..." value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          {filteredProblems.length === 0 ? (
            ...
            filteredProblems.map((problem) => (
              <div className={`problem problem-${safeText(problem.difficulty).toLowerCase()}`} key={problem.id}>
                <div className="problem-header">
                  <div>
                    ...
                    <p className="tags-row">
                      <span className={`tag ${safeText(problem.difficulty).toLowerCase()}`}>{problem.difficulty}</span>
                  ...
                  <div className="actions">
                    <button type="button" onClick={() => toggleFavorite(problem.id)}>{problem.favorite ? "Unstar" : "Star"}</button>
                    <button type="button" onClick={() => toggleRevision(problem.id)}>{problem.revision ? "Done" : "Revise"}</button>
                    <button type="button" onClick={() => editProblem(problem)}>Edit</button>
                    <button type="button" className="delete" onClick={() => deleteProblem(problem.id)}>Delete</button>
                  </div>
                </div>
